...
/*
   it takes ~8.5 sec to cross 435 cm
   so ~51cm per sec.
   ~102cm for 2 sec.
*/
...
void setup() {
  ...
  camServo.write(45, 50, true);

  //basket servo
  basketServo.attach(basket_servo);
  basketServo.write(90, 50, true);
...
void auto_bot() {
  ...
  //test
  /*int rand_array2[] = {-1,0,45,90};
  int camAngle = rand_array2[random(0,4)];
  ...
  if (camAngle < 0) {
    ...
    if (movement == forward) {
      while (timeDiff < moveTime) {
        if (check_distance(obs_trigPin_1, obs_echoPin_1) > 30 &&
      ...
      while (timeDiff < rotateTime) {
        if (check_distance(obs_trigPin_2, obs_echoPin_2) > 30) {
          move_left();
        }
        else {
          reset_relay();
...
void grab_object() {
  ...
  delay(1000);
  servo6.write(55, 50, true);
  delay(1000);
  servo5.write(0, 50, true);
  delay(1000);
  servo3.write(165, 30, true);    //previous: 170
  delay(1000);
  
  servo4.write(180, 50, true);
  delay(1000);
...
int cam_angle() {
  ...
  delay(10000);                //fix delay later if needed
  if (check_type() == 1) {
    camAngle = 45;
  ...
  camServo.write(45,0,true);  //reset camServo
...
int check_type() {
  if (digitalRead(glass) == HIGH) {
    ...
    //rotate basket servo
    basketServo.write(0,90,true);
    return 1;
  }
  else if (digitalRead(paper) == HIGH) {
    //rotate basket servo
    basketServo.write(90,90,true);
    return 1;
  }
  else if (digitalRead(plastic) == HIGH) {
    //rotate basket servo
    basketServo.write(180,90,true);
    return 1;
  }
  else {
    return 0;
...
int check_default_hand() {
  if (servo1.read() == 90 && servo2.read() == 90 && servo3.read() == 90 &&
      servo4.read() == 90 && servo5.read() == 90 && servo6.read() == 90) {
    return 1;
  } else {
    return 0;
  }
